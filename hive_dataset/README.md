...
You can generate mixed audio from either local JSON files or directly from HuggingFace Hub:

<details>
<summary>🌐 <b>Option 1: From HuggingFace Hub (Recommended)</b></summary>

```bash
# Load metadata directly from HuggingFace Hub
python mix_from_metadata/mix_from_metadata.py \
    --hf_dataset ShandaAI/Hive \
    --output_dir ./hive_dataset \
    --dataset_paths mix_from_metadata/dataset_paths.json \
    --num_processes 16
...
# Streaming mode (recommended for large datasets, saves memory)
python mix_from_metadata/mix_from_metadata.py \
    --hf_dataset ShandaAI/Hive \
    --output_dir ./hive_dataset \
    --dataset_paths mix_from_metadata/dataset_paths.json \
...
<details>
<summary>📁 <b>Option 2: From Local Parquet Files</b></summary>

```bash
# If you have downloaded the Parquet files locally
python mix_from_metadata/mix_from_metadata.py \
...
<details>
<summary>📄 <b>Option 3: From Local JSON Files (Legacy)</b></summary>

```bash
# Original format using local JSON metadata files
python mix_from_metadata/mix_from_metadata.py \
...
|----------|-------------|
| `--hf_dataset` | HuggingFace dataset name or local Parquet path |
| `--metadata_dir` | Path to local JSON metadata (legacy format) |
| `--output_dir` | Output directory for generated audio |
| `--dataset_paths` | JSON file mapping dataset prefixes to local paths |
| `--num_processes` | Number of parallel processes (default: 8) |
| `--hf_split` | Specific split to process: train/validation/test |
| `--streaming` | Enable streaming mode for large datasets |
| `--batch_size` | Samples per tar file (default: 1000) |
